...
import org.blaze.flink.arrow.vectors.*;
import org.blaze.flink.arrow.writers.*;
...
public final class ArrowUtils {
    ...
    private static byte[] readNextBatch(ReadableByteChannel channel) throws IOException {
        ...
        if (metadata.getMessage().headerType() == MessageHeader.RecordBatch) {
            ...
            ByteArrayOutputStreamWithPos baos =
                    new ByteArrayOutputStreamWithPos((int) (8 + metadata.getMessageLength() + bodyLength));
    ...
    public static CustomIterator<byte[]> collectAsPandasDataFrame(Table table, int maxArrowBatchSize) throws Exception {
        ...
        VectorSchemaRoot root = VectorSchemaRoot.create(toArrowSchema(rowType), allocator);
    ...
    private static boolean isStreamingMode(Table table) {
        ...
        if (tableEnv instanceof TableEnvironmentImpl) {
            final RuntimeExecutionMode mode = tableEnv.getConfig().get(ExecutionOptions.RUNTIME_MODE);
            if (mode == RuntimeExecutionMode.AUTOMATIC) {
                throw new RuntimeException(String.format("Runtime execution mode '%s' is not supported yet.", mode));
            }
